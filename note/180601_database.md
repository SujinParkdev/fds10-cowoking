...
주기 전반을 체계적이고 서술적이며 **정량적**으로 다루는 학문
...
- 개발운영 (Development Operations)
	- 개발과 운영이 합쳐져서 운영되는 형태가 많이 등장
	- 비용절감 목적에서 시작했으나, 통합 운영으로 인한 협업의 편리함
...
- 개발 서버와 운영 서버의 환경(**프로그램 버젼** 등)을 맞춰주는게 중요함
...
- 요구사항 유도(수집): 대화를 통해 요구사항을 결정하는 작업
  - 가능한 세세하고 다양하게 스무고개해서 니즈를 수집해야함
...
- web은 1차원 timeline 형태로 uc diagram 작성
- user stories는 기술 완성도를 릴리즈별로 어디까지 구현할 것인지를 정함
...
- '~할 수 있다'의 가능성의 표시가 아니라 '~해야 한다 or ~하지 말아야 한다'로 작성
---
## System Requirements
...
- 적당히 큰 그림을 그릴 수 있을 정도면 충분함(핵심사항에 대한 정리만 되면, 시작 후에 추가, 수정 해나가는 방식으로...)

---
과제 : 트립어드바이저 https://www.tripadvisor.co.kr/ 
  - 호텔, 오락거리 두개의 메인메뉴와 회원가입, 로그인, 검색에 제한하여 리버스 엔지니어링 할것.
...
>대규모의 인원이 모여 하는 국내 대기업의 일반적인 모델
...
>요구사항 접수 전에 기본 사항만으로 프로토타입을 만들어서 보여주면서 요구를 접수하는 방식
...
>구현하고자 하는 기능에 대해 테스트코드를 먼저 만들고 기능 구현 후에 즉시 테스트
